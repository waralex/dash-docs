...
parse_jsonstring <- function(data, shape=NULL, scale=1){
  ...
  flag <- function(k)
  {
    ...
    {
      scale = scaleX[k]
      return(indices_of_rect(left[k], top[k], width[k], height[k], scale=scale))
    }
    if (types[k]=="line")
    {
      scale = scaleX[k]
      return(indices_of_line(left[k], top[k], width[k], height[k],shape, scale=scale))
    }
